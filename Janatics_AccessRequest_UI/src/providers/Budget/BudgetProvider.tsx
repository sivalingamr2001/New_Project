...
function BudgetProvider({ children }: BudgetProviderProps) {
  ...
  const fetchBudgetRecords = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch budget records');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const fetchBudgetRecord = useCallback(async (id: string) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch budget record');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const createBudgetRecord = useCallback(async (record: Omit<BudgetRecord, 'id'>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to create budget record');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const updateBudgetRecord = useCallback(async (id: string, updates: Partial<BudgetRecord>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to update budget record');
    } finally {
      setLoading(false);
    }
  }, [activeRecord]);
  ...
  const deleteBudgetRecord = useCallback(async (id: string) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to delete budget record');
    } finally {
      setLoading(false);
    }
  }, [activeRecord]);
